...
async function main() {
  ...
  // 定义任务
  const task = 'go to https://en.wikipedia.org/wiki/Banana and click on buttons on the wikipedia page to go as fast as possible from banna to Quantum mechanics';

  // 创建代理实例
  const agent = new Agent({
    task, llm, useVision: false, initialActions: [
      //{'open_tab': {'url': 'https://www.baidu.com'}}
    ], plannerLlm: undefined ,browser});
